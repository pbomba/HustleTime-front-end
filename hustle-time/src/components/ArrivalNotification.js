...
const ArrivalNotification = (props) => {
	...
	if ( sb.length === 0 ) {
		...
		return(
			<div onClick={() => props.dispatchedActiveMarker(props.station.name)} style={{ display: 'flex', height:30, margin:5, padding: 8, color:'navy', backgroundColor:'white', fontSize:10, lineHeight: 1.6, position: 'relative', paddingBottom: 10, textAlign: 'left' }}>
				<div style={{width: '50%', fontSize:14 }} >{stationName} </div>
				<div style={{height: '100%'}}>Northbound: {nba}<br/>
				Southbound: {sba}
				</div>
			</div>
			)
...
export default connect(null, mapDispatchToProps)(ArrivalNotification)
